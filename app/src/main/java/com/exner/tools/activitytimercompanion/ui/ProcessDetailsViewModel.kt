...
class ProcessDetailsViewModel @AssistedInject constructor(
    ...
    private val _currentCategory =
        MutableStateFlow(TimerProcessCategory(name = "None", backgroundUri = null, uid = -1L))
    val currentCategory: StateFlow<TimerProcessCategory>
        get() = _currentCategory

    private val _backgroundUri: MutableLiveData<String> =
        MutableLiveData("https://fototimer.net/assets/activitytimer/bg-default.png")
    ...
    private fun updateCategoryId(id: Long) {
        if (id == -1L) {
            _currentCategory.value =
                TimerProcessCategory(name = "None", backgroundUri = null, uid = -1L)
        } else {
            viewModelScope.launch {
                _currentCategory.value = repository.getCategoryById(id) ?: TimerProcessCategory(
                    name = "None",
                    backgroundUri = null,
                    uid = -1L
                )
